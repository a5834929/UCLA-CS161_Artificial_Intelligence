...
(defun most-one-helper (n c i k)
    (cond   ((= c k) nil)
            ((>= (+ c i) k) (list (cons (- 0 (node2var n c k)) (list (- 0 (node2var n (+ c i) k))))))
            (T  (cons  (cons (- 0 (node2var n c k)) (list (- 0 (node2var n (+ c i) k)))) (most-one-helper n c (+ i 1) k)))
    )
)
...
(defun at-most-one-color (n c k)
    (cond   ((= c k) nil)
            (T  (append (most-one-helper n c 1 k) (at-most-one-color n (+ c 1) k)))
    )
)
...
(defun edge-clauses-helper (e c k)
    (let    ((v1 (car e))
            (v2 (cadr e)))

            (cond   ((= c 0) nil)
                    (T (cons (list (- 0 (node2var v1 c k)) (- 0 (node2var v2 c k))) (edge-clauses-helper e (- c 1) k)))
    ))
)
...
; (print (node2var 1 1 3))
; (print (at-least-one-color 1 1 3))
; (print (generate-edge-clauses '(1 2) 3))
; (print (edge-clauses-helper '(1 2) 3 3))
; (print (at-most-one-color 1 1 3))
; (print (most-one-helper 1 1 1 3))
; (print (most-one-helper 1 2 1 3))
; (print (most-one-helper 1 3 1 3))
; (print (node-clauses-helper 1 1 3))
; (print (generate-node-clauses 2 4))
; (graph-coloring-to-sat "graph2.txt" "sat2_3.txt" 3)
